            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  Após o upload dos documentos, o LLM analisa o conteúdo e gera automaticamente a estrutura de ontologia adequada para simulação de opinião pública (tipos de entidade + tipos de relação)
                </div>
              </div>

              <!-- Progresso de geração de ontologia -->
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Descrição da interface</div>
                <div class="detail-content">
                  Com base na ontologia gerada, os documentos são divididos em blocos e a API Zep é chamada para construir o grafo de conhecimento, extraindo entidades e relações
                </div>
              </div>
...
const ontologyProgress = ref(null) // progresso de geração de ontologia
...
const goToNextStep = () => {
  // TODO: Ir para a etapa de Configuração do Ambiente
...
// Inicialização - lidar com novo projeto ou carregar projeto existente
const initProject = async () => {
  const paramProjectId = route.params.projectId

  if (paramProjectId === 'new') {
    // Novo projeto: obter dados pendentes de upload do store
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'Nenhum arquivo pendente para upload. Volte à página inicial e tente novamente.'
    loading.value = false
    return
  }
...
// Iniciar polling dos dados do grafo
...
// Parar polling dos dados do grafo
...
const fetchGraphData = async () => {
  try {
    // Primeiro obter informações do projeto para recuperar graph_id
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      if (graphResponse.success && graphResponse.data) {
        ...
        // Atualizar renderização quando os dados mudarem
...
// Iniciar polling do status da tarefa
const startPollingTask = (taskId) => {
  // Executar uma consulta imediatamente
  pollTaskStatus(taskId)

  // Em seguida, polling periódico
...
const renderGraph = () => {
  ...
  // Criar mapa de nós para buscar nomes
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  // Desenhar arestas (incluindo linha larga transparente clicável)
  ...
  simulation.on('tick', () => {
    // Atualizar posição de todas as arestas (incluindo linha visível e área de clique transparente)
...
/* Exibição de relação nos detalhes da aresta */
.edge-relation {
  display: flex;
  align-items: center;
  flex-wrap: wrap;
