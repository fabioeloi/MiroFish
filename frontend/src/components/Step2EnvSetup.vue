    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Criação de nova instância de simulação, carregando templates de parâmetros do mundo simulado
          </p>
      ...
      <!-- Step 02: Personas de Agentes -->
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">02</span>
            <span class="step-title">Gerar Personas de Agentes</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combinando o contexto, ferramentas são chamadas automaticamente para mapear entidades e relações do grafo de conhecimento, inicializar indivíduos de simulação e dotá-los de comportamentos e memórias únicas com base nas sementes da realidade
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            <div class="stat-card">
              ...
              <span class="stat-label">Total de Agentes Atual</span>
            </div>
            <div class="stat-card">
              <span class="stat-value">{{ expectedTotal || '-' }}</span>
              <span class="stat-label">Total de Agentes Esperado</span>
            </div>
            <div class="stat-card">
              <span class="stat-value">{{ totalTopicsCount }}</span>
              <span class="stat-label">Tópicos Associados às Sementes da Realidade</span>
            </div>
          </div>
      ...
      <!-- Step 03: Gerar Configuração de Simulação Dupla Plataforma -->
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">03</span>
            <span class="step-title">Gerar Configuração de Simulação Dupla Plataforma</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            O LLM define de forma inteligente a velocidade do tempo do mundo, algoritmos de recomendação, períodos de atividade de cada indivíduo, frequência de postagens, acionamento de eventos e outros parâmetros com base nos requisitos de simulação e nas sementes da realidade
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              ...
              <div class="time-periods">
                ...
                <div class="period-item">
                  <span class="period-label">Horário de Vale</span>
            ...
            <div class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">Configuração de Algoritmos de Recomendação</span>
              </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Com base na direção narrativa, eventos de ativação inicial e tópicos em alta são gerados automaticamente para guiar o estado inicial do mundo simulado
          </p>
      ...
      <div class="step-card" :class="{ ‘active’: phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">O ambiente de simulação está pronto, a simulação pode ser iniciada</p>

          <!-- Configuração de Rodadas de Simulação - exibida somente após a geração da configuração e o cálculo das rodadas -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Definição de Rodadas de Simulação</span>
                <span class="section-desc">MiroFish planeja automaticamente a simulação de <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || ‘-’ }}</span> horas da realidade; cada rodada representa <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || ‘-’ }}</span> minutos do tempo real</span>
              </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">Na primeira execução, recomenda-se fortemente mudar para o ‘Modo Personalizado’ para reduzir as rodadas de simulação e visualizar rapidamente os resultados ➝</p>
                    </div>
                  </div>
                </div>
              </div>
...
const useCustomRounds = ref(false) // padrão: usar rodadas geradas automaticamente
const customMaxRounds = ref(40)   // padrão recomendado: 40 rodadas

// Watch stage to update phase
watch(currentStage, (newStage) => {
  if (newStage === 'Gerando Personas de Agentes' || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === 'Gerando Configuração de Simulação' || newStage === 'generating_config') {
    phase.value = 2
    // Entrando na fase de geração de configuração, iniciar polling
    if (!configTimer) {
      addLog('Iniciando geração da configuração de simulação para as duas plataformas...')
      startConfigPolling()
    }
  } else if (newStage === 'Preparando Scripts de Simulação' || newStage === 'copying_scripts') {
    phase.value = 2 // ainda pertence à fase de configuração
  }
})

// Calcula as rodadas geradas automaticamente a partir da configuração (sem valores padrão fixos)
const autoGeneratedRounds = computed(() => {
  ...
  // Garante que o número máximo de rodadas não seja menor que 40 (valor recomendado), evitando intervalos inválidos no slider
...
// Obtém o username correspondente ao agent_id
...
// Calcula o total de tópicos associados de todas as personas
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // Rodadas personalizadas pelo usuário, passa o parâmetro max_rounds
    params.maxRounds = customMaxRounds.value
    addLog(`Iniciando simulação, rodadas personalizadas: ${customMaxRounds.value} rodadas`)
  } else {
    // Usuário optou por manter as rodadas geradas automaticamente, não passa max_rounds
    addLog(`Iniciando simulação, usando rodadas automáticas: ${autoGeneratedRounds.value} rodadas`)
  }
...
// Inicia automaticamente a preparação da simulação
const startPrepareSimulation = async () => {
  ...
  // Marca a primeira etapa como concluída, inicia a segunda
  phase.value = 1
  addLog(`Instância de simulação criada: ${props.simulationId}`)
  addLog('Preparando o ambiente de simulação...')
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Preparação anterior concluída detectada, utilizando diretamente')
      ...
      // Define imediatamente o total esperado de Agentes (obtido do retorno da API prepare)
